...
class TestServicesUtilityFunctions:
    ...
    @pytest.mark.skipif(not services_module_available, reason="Services module not importable")
    ...
    def test_decrypt_device_mqtt_credentials_success(self):
        ...
        mock_device.connected_configuration.mqtt.local_broker_credentials = "encrypted_creds"
    ...
    def test_find_cloud_coordinators_with_cloud_entries(self):
        ...
        mock_cloud_entry.data = {"discovery_method": "cloud", "email": "test@example.com"}
        ...
        mock_hass.data = {
            "hass_dyson": {
                mock_cloud_entry.entry_id: MagicMock()
            }
        }
...
class TestAsyncServiceFunctions:
    ...
    async def test_get_device_data_from_coordinator_only_sanitized(self):
        ...
        from custom_components.hass_dyson.services import _get_device_data_from_coordinator_only
        ...
        with patch("custom_components.hass_dyson.services._create_sanitized_device_info_from_coordinator") as mock_create:
            mock_create.return_value = {"serial": "***HIDDEN***", "name": "Test Device"}

            result = await _get_device_data_from_coordinator_only(mock_coordinator, sanitize=True)
    ...
    async def test_get_device_data_from_coordinator_only_detailed(self):
        ...
        with patch("custom_components.hass_dyson.services._create_detailed_device_info_from_coordinator") as mock_create:
            mock_create.return_value = {"serial": "TEST123", "name": "Test Device", "details": "full"}

            result = await _get_device_data_from_coordinator_only(mock_coordinator, sanitize=False)
    ...
    async def test_build_device_data_from_live_api_sanitized(self):
        ...
        from custom_components.hass_dyson.services import _build_device_data_from_live_api
        ...
        result = await _build_device_data_from_live_api(enhanced_devices, "test@example.com", sanitize=True)
...
class TestErrorHandlingScenarios:
    ...
    def test_decrypt_device_mqtt_credentials_value_error(self):
        ...
        mock_cloud_client = MagicMock()
        mock_cloud_client.decrypt_local_credentials.side_effect = ValueError("Invalid format")

        mock_device = MagicMock()
        mock_device.serial_number = "TEST123"
        mock_device.connected_configuration.mqtt.local_broker_credentials = "invalid_creds"
