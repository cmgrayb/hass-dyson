...
class TestSwitchPlatformSetup:
    ...
    async def test_async_setup_entry_creates_switches(
        ...
        # Arrange
        pure_mock_hass.data[DOMAIN] = {pure_mock_config_entry.entry_id: pure_mock_coordinator}
        ...
        # Act
        result = await async_setup_entry(pure_mock_hass, pure_mock_config_entry, mock_add_entities)
...
class TestDysonNightModeSwitch:
    ...
    def test_is_on_when_night_mode_active(self, pure_mock_coordinator, pure_mock_sensor_entity):
    ...
    def test_is_on_when_night_mode_inactive(self, pure_mock_coordinator, pure_mock_sensor_entity):
...
class TestDysonContinuousMonitoringSwitch:
    ...
    def test_initialization(self, pure_mock_coordinator, pure_mock_sensor_entity):
        """Test switch initialization."""
        switch = pure_mock_sensor_entity(DysonContinuousMonitoringSwitch, pure_mock_coordinator)

        assert switch.coordinator == pure_mock_coordinator
        assert switch.unique_id == f"{pure_mock_coordinator.serial_number}_continuous_monitoring"

    def test_is_on_when_monitoring_active(self, pure_mock_coordinator, pure_mock_sensor_entity):
        """Test switch reports on when continuous monitoring is active."""
        pure_mock_coordinator.data["product-state"]["rhtm"] = "ON"  # Correct field is rhtm
        switch = pure_mock_sensor_entity(DysonContinuousMonitoringSwitch, pure_mock_coordinator)
    ...
    def test_is_on_when_monitoring_inactive(self, pure_mock_coordinator, pure_mock_sensor_entity):
        """Test switch reports off when continuous monitoring is inactive."""
        pure_mock_coordinator.data["product-state"]["rhtm"] = "OFF"  # Correct field is rhtm
        switch = pure_mock_sensor_entity(DysonContinuousMonitoringSwitch, pure_mock_coordinator)
    ...
    async def test_turn_on(self, pure_mock_coordinator, pure_mock_sensor_entity):
        ...
        pure_mock_coordinator.device.set_continuous_monitoring.assert_called_once_with(True)
    ...
    async def test_turn_off(self, pure_mock_coordinator, pure_mock_sensor_entity):
        ...
        pure_mock_coordinator.device.set_continuous_monitoring.assert_called_once_with(False)
...
class TestDysonHeatingSwitch:
    ...
    def test_is_on_when_heating_active(self, pure_mock_coordinator, pure_mock_sensor_entity):
    ...
    def test_is_on_when_heating_inactive(self, pure_mock_coordinator, pure_mock_sensor_entity):
    ...
    async def test_turn_on(self, pure_mock_coordinator, pure_mock_sensor_entity):
        ...
        pure_mock_coordinator.device.set_heating_mode.assert_called_once_with("HEAT")  # Correct argument
    ...
    async def test_turn_off(self, pure_mock_coordinator, pure_mock_sensor_entity):
        ...
        pure_mock_coordinator.device.set_heating_mode.assert_called_once_with("OFF")  # Correct argument
...
class TestDysonFirmwareAutoUpdateSwitch:
    ...
    def test_initialization(self, pure_mock_coordinator, pure_mock_sensor_entity):
        """Test switch initialization."""
        switch = pure_mock_sensor_entity(DysonFirmwareAutoUpdateSwitch, pure_mock_coordinator)

        assert switch.coordinator == pure_mock_coordinator
        assert switch.unique_id == f"{pure_mock_coordinator.serial_number}_firmware_auto_update"

    def test_is_on_when_auto_update_enabled(self, pure_mock_coordinator, pure_mock_sensor_entity):
        """Test switch reports on when firmware auto update is enabled."""
        pure_mock_coordinator.firmware_auto_update_enabled = True  # Use coordinator property
        switch = pure_mock_sensor_entity(DysonFirmwareAutoUpdateSwitch, pure_mock_coordinator)

        # Firmware auto update uses is_on property directly, not _attr_is_on
        assert switch.is_on is True

    def test_is_on_when_auto_update_disabled(self, pure_mock_coordinator, pure_mock_sensor_entity):
        """Test switch reports off when firmware auto update is disabled."""
        pure_mock_coordinator.firmware_auto_update_enabled = False  # Use coordinator property
        switch = pure_mock_sensor_entity(DysonFirmwareAutoUpdateSwitch, pure_mock_coordinator)
    ...
    async def test_turn_on(self, pure_mock_coordinator, pure_mock_sensor_entity):
        ...
        pure_mock_coordinator.async_set_firmware_auto_update = AsyncMock(return_value=True)
        switch = pure_mock_sensor_entity(DysonFirmwareAutoUpdateSwitch, pure_mock_coordinator)

        await switch.async_turn_on()

        pure_mock_coordinator.async_set_firmware_auto_update.assert_called_once_with(True)
    ...
    async def test_turn_off(self, pure_mock_coordinator, pure_mock_sensor_entity):
        ...
        pure_mock_coordinator.async_set_firmware_auto_update.assert_called_once_with(False)
...
class TestSwitchErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_switch_handles_device_error_gracefully(self, pure_mock_coordinator, pure_mock_sensor_entity):
        """Test switch handles device errors gracefully by logging."""
        pure_mock_coordinator.device.set_night_mode = AsyncMock(side_effect=Exception("Device error"))
        switch = pure_mock_sensor_entity(DysonNightModeSwitch, pure_mock_coordinator)
    ...
    def test_switch_with_none_device_state(self, pure_mock_coordinator, pure_mock_sensor_entity):
    ...
    def test_switch_coordinator_update_with_missing_data(self, pure_mock_coordinator, pure_mock_sensor_entity):
    ...
    async def test_setup_entry_missing_coordinator(self, pure_mock_hass):
        ...
        with pytest.raises(KeyError):
            await async_setup_entry(pure_mock_hass, mock_config_entry, mock_add_entities)


class TestSwitchMissingCoverage:
    """Test scenarios to improve switch platform coverage."""

    def test_switch_entity_inheritance(self, pure_mock_coordinator, pure_mock_sensor_entity):
        """Test switch entities inherit from proper base classes."""
        night_switch = pure_mock_sensor_entity(DysonNightModeSwitch, pure_mock_coordinator)
        heating_switch = pure_mock_sensor_entity(DysonHeatingSwitch, pure_mock_coordinator)
    ...
    def test_switch_unique_ids_are_unique(self, pure_mock_coordinator, pure_mock_sensor_entity):
        """Test all switch entities have unique IDs."""
        switches = [
            pure_mock_sensor_entity(DysonNightModeSwitch, pure_mock_coordinator),
            pure_mock_sensor_entity(DysonContinuousMonitoringSwitch, pure_mock_coordinator),
            pure_mock_sensor_entity(DysonHeatingSwitch, pure_mock_coordinator),
            pure_mock_sensor_entity(DysonFirmwareAutoUpdateSwitch, pure_mock_coordinator),
        ]

        unique_ids = [switch.unique_id for switch in switches]
        assert len(unique_ids) == len(set(unique_ids)), "All switch unique IDs should be unique"

    def test_switch_translation_keys_are_unique(self, pure_mock_coordinator, pure_mock_sensor_entity):
        ...
        translation_keys = [getattr(switch, 'translation_key', None) for switch in switches]
        # Remove None values
        translation_keys = [key for key in translation_keys if key is not None]
        assert len(translation_keys) == len(set(translation_keys)), "All switch translation keys should be unique"

    def test_all_switches_support_turn_on_off(self, pure_mock_coordinator, pure_mock_sensor_entity):
    ...
    def test_switches_have_proper_device_info(self, pure_mock_coordinator, pure_mock_sensor_entity):
    ...
    def test_switch_state_with_various_device_states(self, pure_mock_coordinator, pure_mock_sensor_entity):
    ...
    def test_setup_with_coordinator_having_no_device(self, pure_mock_hass, pure_mock_config_entry, pure_mock_coordinator):
    ...
    def test_switch_coordinator_data_updates(self, pure_mock_coordinator, pure_mock_sensor_entity):
