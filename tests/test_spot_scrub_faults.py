...
def test_known_code_is_named(coordinator, device):
...
def test_unknown_code_is_shown_raw(coordinator, device):
...
def test_no_faults_reads_none_string(coordinator, device):
...
def test_sensor_unreported_when_field_absent(coordinator, device):
...
def test_problem_sensor_off_for_status_only(coordinator, device):
    """Status codes must not leave a permanent problem state."""
    device._state_data = {"activeFaults": [STATUS_FAULT]}
    sensor = DysonRobotActionRequiredSensor(coordinator)
...
def test_problem_sensor_on_and_lists_codes(coordinator, device):
...
def test_problem_sensor_unknown_without_a_device(coordinator):
