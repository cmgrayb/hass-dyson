...
class DysonFindFollowModeSelect(DysonEntity, SelectEntity):
    ...
    room, directing airflow toward them.  It is controlled via the ``soon``
    state key and detected at runtime by the presence of that key in the
    device's product-state (no dedicated capability flag exists).
    ...
    State reading uses the ``soon`` key directly.  After a ``SCAN`` command
    the device automatically transitions ``soon`` back to ``ON``, so the
    select returns to "Find+Follow" without any HA-side intervention.
    """

    coordinator: DysonDataUpdateCoordinator

    # Maps the ``soon`` device value to the HA option string.
    _SOON_TO_OPTION: dict[str, str] = {
        "OFF": "Off",
        "ON": "Find+Follow",
        "SCAN": "Scanning",
    }
    # Maps the HA option string back to the ``soon`` command value.
    _OPTION_TO_SOON: dict[str, str] = {
        "Off": "OFF",
        "Find+Follow": "ON",
        "Scanning": "SCAN",
    }
    ...
    def __init__(self, coordinator: DysonDataUpdateCoordinator) -> None:
        """Initialize the Find+Follow mode select."""
        super().__init__(coordinator)
        self._attr_unique_id = f"{coordinator.serial_number}_find_follow_mode"
        self._attr_translation_key = "find_follow_mode"
        self._attr_icon = "mdi:account-eye"
        self._attr_options = ["Off", "Find+Follow", "Scanning"]
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        self._attr_current_option = self._SOON_TO_OPTION.get(soon, "Off")
        super()._handle_coordinator_update()

    async def async_select_option(self, option: str) -> None:
        """Handle a Find+Follow mode selection."""
        if not self.coordinator.device:
            return

        soon_value = self._OPTION_TO_SOON.get(option)
        if soon_value is None:
            _LOGGER.error(
                "Unknown Find+Follow option '%s' for %s",
                option,
                self.coordinator.serial_number,
            )
            return
        ...
        try:
            await self.coordinator.device.set_find_follow(soon_value)

            # Optimistic update for responsive UI.
            # For SCAN the device will transition back to ON automatically;
            # showing Scanning momentarily is intentional feedback.
            self._attr_current_option = option
            self.async_write_ha_state()
            ...
            _LOGGER.error(
                "Communication error setting Find+Follow mode to '%s' for %s: %s",
                option,
                self.coordinator.serial_number,
                err,
            )
            ...
            _LOGGER.error(
                "Invalid Find+Follow option '%s' for %s: %s",
                option,
                self.coordinator.serial_number,
                err,
            )
            ...
            _LOGGER.error(
                "Unexpected error setting Find+Follow mode to '%s' for %s: %s",
                option,
                self.coordinator.serial_number,
                err,
            )
